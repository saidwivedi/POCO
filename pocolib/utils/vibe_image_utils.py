...
def points_inside_bbox(points, bbox, num_valid_points=10):
    ...
    valid_points = ((points >= bbox_xy[:2]) & (points <= bbox_xy[2:])).all(1)
    if len(valid_points.nonzero()[0]) > num_valid_points:
        return True
