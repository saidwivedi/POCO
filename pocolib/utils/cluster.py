...
from ..utils.os_utils import get_project_root

CONDOR_ROOT = '/is/cluster/fast/sdwivedi/condor/poco_condor'
GPUS = {
...
def execute_task_on_cluster(
    ...
    submission = f'executable = {ROOT}/scripts/{method}/{exp_name}_run.sh\n' \
                 'arguments = $(Process)\n' \
    ...
    with open(f'{ROOT}/scripts/{method}/{exp_name}_submit.sub', 'w') as f:
        f.write(submission)
    ...
    with open(f'{ROOT}/scripts/{method}/{exp_name}_run.sh', 'w') as f:
        f.write(bash)

    os.chmod(f'{ROOT}/scripts/{method}/{exp_name}_run.sh', stat.S_IRWXU)

    cmd = ['condor_submit_bid', f'{bid_amount}', f'{ROOT}/scripts/{method}/{exp_name}_submit.sub']
