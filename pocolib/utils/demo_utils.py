...
def download_youtube_clip(url, download_folder):
    ...
    return yt.download(output_path=download_folder)
...
def xyhw_to_xyxy(bbox_xyhw):
    x_center, y_center, width, height = bbox_xyhw
    ...
    for person_id, person_data in vibe_results.items():
        for idx, frame_id in enumerate(person_data['frame_ids']):
            frame_results[frame_id][person_id] = {
                ...
                'var': person_data['var'][idx] if len(person_data['var']) > 1 else None,
                'var_global': person_data['var_global'][idx] \
                              if len(person_data['var_global']) > 1 else None,
            }
            if 'verts_interp' in person_data.keys():
                frame_results[frame_id][person_id].update({
                    'verts_interp': person_data['verts_interp'][idx],
                    'frames_interp': person_data['frames_interp'][idx],
                })
            if 'verts_opt' in person_data.keys():
                frame_results[frame_id][person_id].update({
                    'verts_opt': person_data['verts_opt'][idx],
                    'cam_opt': person_data['orig_cam_opt'][idx],
                    'joints2d_op': person_data['joints2d_op'][idx],
                })
