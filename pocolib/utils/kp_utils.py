...
def associate_keypoints_with_bbox(keypoints, bounding_boxes, img_size):
    bbox_kps = []
    dist = []
    max_dist = np.sqrt(img_size[0]**2 + img_size[1]**2)
    
    for bbox in bounding_boxes:
        x_center, y_center, _, _ = bbox
        bbox_centroid = (x_center, y_center)
        min_distance = np.inf
        associated_keypoints = []

        if isinstance(keypoints, dict):
            _, keypoints = keypoints.items()

        for keypoints_set in keypoints:
            distance_sum = 0
            valid_keypoints_set = keypoints_set[keypoints_set[:,-1]> 0.3]
            for keypoint in valid_keypoints_set:
                keypoint_x, keypoint_y, _ = keypoint
                distance = np.sqrt((keypoint_x - bbox_centroid[0])**2 + (keypoint_y - bbox_centroid[1])**2)
                distance_sum += distance

            average_distance = distance_sum / (len(valid_keypoints_set) * max_dist)

            if average_distance < min_distance:
                min_distance = average_distance
                associated_keypoints = keypoints_set

        if associated_keypoints is not None:
            bbox_kps.append(associated_keypoints)

        dist.append(min_distance)

    return bbox_kps, dist
