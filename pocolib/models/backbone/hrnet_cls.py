...
def get_default_cfgs(pretrained):
...
def hrnet_w48_cls(
    ...
    pretrained_ckpt='data/pretrained_models/pose_coco/pose_hrnet_w48_cls.pth',
    cfg = get_default_cfgs(pretrained_ckpt)
    model = HighResolutionNet(cfg)
    model.init_weights(cfg['MODEL']['PRETRAINED'][0])
    return model
