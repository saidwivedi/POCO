...
class pare_head(nn.Module):
    def __init__(
            ...
            iter_residual=False,
            num_iterations=3,
            shape_input_type='feats.shape.cam', # 'feats.pose.shape.cam'
            pose_input_type='feats.self_pose.shape.cam', # 'feats.neighbor_pose_feats.all_pose.self_pose.neighbor_pose.shape.cam'
        ...
        super(pare_head, self).__init__()
        self.backbone = backbone
        self.uncert_layer = uncert_layer
        self.uncert_act = uncert_act
        ...
        if self.use_hmr_regression:
            ...
            if self.uncert_layer == 'same_branch_v1':
                self.uncert_mlp = self._get_pose_mlp(num_joints=num_joints, output_size=1)
                if self.uncert_act == 'sigmoid':
                    self.uncert_act_sig = nn.Sigmoid()
                elif self.uncert_act == 'relu':
                    self.uncert_act_relu == nn.ReLU(inplace=True)
                elif self.uncert_act == 'softplus':
                    self.uncert_act_softplus = nn.Softplus()
    ...
    def forward(self, features, gt_segm=None):
        ...
        pred_pose, pred_shape, pred_cam, pred_uncert = self._get_final_preds(
        ...
        if self.uncert_layer == 'same_branch_v1':
            output.update({
                'var_pose': pred_uncert,
            })
        elif self.uncert_layer == 'diff_branch':
            output.update({
                'uncert_feat': point_local_feat.reshape(batch_size, -1)
            })
        elif self.uncert_layer == 'diff_branch_lc2d':
            output.update({
                'uncert_feat': point_local_feat.unsqueeze(-1)
            })
    ...
    def _pare_get_final_preds(self, pose_feats, cam_shape_feats, init_pose, init_shape, init_cam):
        ...
        if self.iterative_regression:
            ...
            pred_uncert = None
            if self.uncert_layer == 'same_branch_v1':
                pred_uncert = self.uncert_mlp(pose_feats)
                if self.uncert_act == 'sigmoid':
                    pred_uncert = self.uncert_act_sig(pred_uncert)
                elif self.uncert_act == 'relu':
                    pred_uncert = self.uncert_act_relu(pred_uncert)
                elif self.uncert_act == 'softplus':
                    pred_uncert = self.uncert_act_softplus(pred_uncert)
                pred_uncert = pred_uncert.squeeze(-1).transpose(2, 1).squeeze(-1)
        ...
        return pred_pose, pred_shape, pred_cam, pred_uncert
    ...
    def get_output_channels(self):
        if 'lc2d' in self.uncert_layer:
            return self.num_deconv_filters[-1]
        else:
            return self.num_joints * self.num_deconv_filters[-1]
