...
# Original Code: https://github.com/mkocabas/PARE/blob/master/pare/models/head/smpl_cam_head.py
...
class smplcam_head(nn.Module):
    ...
    def forward(self, rotmat, shape, cam, focal_length,
        ...
        fullimg_cam_t = convert_pare_to_full_img_cam(
            pare_cam=cam.detach().clone(),
        ...
        crop_cam_t = convert_weak_perspective_to_perspective(cam)

        # TODO: Check if cam_rotmat would be unit
        cam_rotmat = torch.eye(3, device=device).unsqueeze(0).expand(batch_size, -1, -1)
        joints2d = perspective_projection(
            joints3d,
            rotation=cam_rotmat,
            translation=fullimg_cam_t,
            cam_intrinsics=cam_intrinsics,
        )

        output['smpl_joints2d'] = joints2d
        output['pred_cam_t'] = crop_cam_t
        output['pred_fullimg_cam_t'] = fullimg_cam_t
